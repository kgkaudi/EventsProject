...
describe("User Service (Success + Edge Cases)", () => {
  ...
  test("login → error", async () => {
    ...
    await expect(userService.login("a@a.com", "123")).rejects.toThrow("Login failed");
  });
  ...
  test("signup → success", async () => {
    ...
    const result = await userService.signup("Kostas", "a@a.com", "StrongPass123!", "user");
  ...
  test("signup → error", async () => {
    ...
    await expect(userService.signup("A", "a@a.com", "123", "user")).rejects.toThrow("Signup failed");
  });
  ...
  test("updateUser → not found", async () => {
    ...
    await expect(userService.updateUser("123", {})).rejects.toThrow("User not found");
  });

  // UPDATE PASSWORD
  test("updatePassword → missing fields", async () => {
    await expect(userService.updatePassword("123", null, null))
  ...
  test("updatePassword → weak password", async () => {
    ...
    await expect(userService.updatePassword("123", "old", "weak"))
  ...
  test("updatePassword → user not found", async () => {
    ...
    userRepository.findById.mockResolvedValue(null);
    await expect(userService.updatePassword("123", "old", "StrongPass123!"))
  ...
  test("updatePassword → wrong current password", async () => {
    ...
    await expect(userService.updatePassword("123", "wrong", "StrongPass123!"))
  ...
  test("updatePassword → success", async () => {
    ...
    const result = await userService.updatePassword("123", "old", "StrongPass123!");
    expect(result.message).toBe("Password updated successfully");
  });

  // DELETE USER
  test("deleteUser → missing password", async () => {
    await expect(userService.deleteUser("admin", "123", null))
      .rejects.toThrow("Password is required");
  });

  test("deleteUser → admin not found", async () => {
    userRepository.findById.mockResolvedValue(null);
    await expect(userService.deleteUser("admin", "123", "pass"))
  ...
  test("deleteUser → wrong admin password", async () => {
    ...
    await expect(userService.deleteUser("admin", "123", "wrong"))
  ...
  test("deleteUser → success", async () => {
    ...
    expect(result.message).toBe("User and all associated events were deleted successfully");
  });

  // UPDATE ROLE
  test("updateRole → invalid role", async () => {
    await expect(userService.updateRole("123", "invalid"))
      .rejects.toThrow("Invalid role");
  });

  test("updateRole → not found", async () => {
    userRepository.updateRole.mockResolvedValue(null);
    await expect(userService.updateRole("123", "admin"))
      .rejects.toThrow("User not found");
  });
